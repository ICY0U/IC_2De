...
[[nodiscard]] ProjectedPoint25D project_world_point(const Vec3& world_position,
                                                    const Camera25DState& camera) noexcept;

// Converts screen-relative ground input (right, forward) into an X/Z direction.
[[nodiscard]] Vec3 camera_ground_direction_to_world(const Vec2& camera_direction,
                                                    const Camera25DState& camera) noexcept;

// Inverse yaw transform used when world-space aim must select a screen-facing
// animation direction.
[[nodiscard]] Vec2 world_ground_direction_to_camera(const Vec2& world_direction,
                                                    const Camera25DState& camera) noexcept;
...
[[nodiscard]] Vec3 canvas_ground_point(Vec2 canvas_point, float ground_elevation, int canvas_width,
                                       int canvas_height, const Camera25DState& camera) noexcept;
...
[[nodiscard]] Vec2 canvas_ground_direction(const Vec3& world_origin, const Vec2& canvas_point,
...
// already draws.
[[nodiscard]] DepthSlicePlan plan_depth_slices(float center_z, float depth_span,
                                               float sprite_height, float pitch_degrees) noexcept;
...
[[nodiscard]] Vec3 canvas_ground_offset_to_world(Vec2 canvas_offset,
                                                 const Camera25DState& camera) noexcept;

// Keeps screen-relative facing separate from the camera-rotated world vector
// used by simulation.
[[nodiscard]] GroundMovement25D camera_ground_movement(Vec2 camera_direction,
                                                       const Camera25DState& camera) noexcept;
