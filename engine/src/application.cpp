...
struct EnemyIntentObservations {
    ...
    void observe_player_damage(
        const std::vector<HealthEvent>& events,
        const EntityUuid player
    ) noexcept {
        for (const HealthEvent& event : events) {
...
[[nodiscard]] RectXZ visible_world_region(
...
[[nodiscard]] float stress_acquisition_range(const NavGridSnapshot& topology) noexcept {
    const float arena_reach =
        std::min(topology.bounds.width, topology.bounds.depth) * 0.5F;
...
[[nodiscard]] CrowdSteeringPlan plan_crowd_steering(const RuntimeScene& scene) {
    const std::vector<EntityUuid> attackers =
        scene.actor_uuids(ScenePhysicsRole::attacker);
...
[[nodiscard]] NavGridBakeSettings navigation_grid_settings(
...
[[nodiscard]] NavPathResult navigation_reference_path(
    ...
    for (const GroundArea& area : ground.areas) {
        ...
        const NavPathResult path = find_nav_path_world(
            grid,
            {area.bounds.x - snapshot.cell_size * 2.0F, center_z},
    ...
    const auto last = std::find_if(
        snapshot.cells.rbegin(), snapshot.cells.rend(),
        [](const NavGridCell& cell) { return cell.walkable; });
...
void register_scene_health_targets(Health& health, const RuntimeScene& scene) {
    ...
    for (const EntityUuid actor : scene.actor_uuids(ScenePhysicsRole::attacker)) {
        if (!health.register_target({
            ...
            throw std::logic_error{"Runtime scene provided a duplicate or invalid attacker health target."};
        }
    }
}

void register_scene_enemy_intents(
    EnemyIntent& intent,
    ...
    for (const EntityUuid actor : scene.actor_uuids(ScenePhysicsRole::attacker)) {
        if (!intent.register_actor({
            ...
            throw std::logic_error{"Runtime scene provided a duplicate or invalid attacker intent."};
        }
    }
}
...
void register_scene_navigation_agents(
    NavAgentSystem& navigation,
...
[[nodiscard]] EnemyStressSpawnSummary spawn_editor_stress_runners(
    RuntimeScene& scene,
    const NavGrid& grid,
    const std::size_t requested_total
) {
    const std::vector<EntityUuid> authored_actors =
        scene.actor_uuids(ScenePhysicsRole::attacker);
    ...
    const float arena_reach =
        std::min(snapshot.bounds.width, snapshot.bounds.depth) * 0.5F;
    const float minimum_spawn_distance = std::max(100.0F, arena_reach * 0.45F);
    const float minimum_spawn_distance_squared =
    ...
    for (std::size_t spawn_index = 0; spawn_index < spawn_count; ++spawn_index) {
        const float angle = first_angle_radians +
                            static_cast<float>(spawn_index) * golden_angle_radians;
        const float radius =
            arena_reach *
            spawn_ring_fractions[spawn_index % spawn_ring_fractions.size()];
        const Vec2 desired{
    ...
    summary.spawned = scene.spawn_actor_copies(
                                ScenePhysicsRole::attacker, positions)
                          .size();
    return summary;
}
...
[[nodiscard]] EditorRuntimeSceneCandidate make_editor_runtime_scene_candidate(
    SceneDefinition definition,
    TextureAssets& textures,
    const std::size_t stress_target_count
) {
    auto grid = std::make_unique<NavGrid>(
        definition.ground(), navigation_grid_settings(definition));
    ...
    EnemyStressSpawnSummary stress = spawn_editor_stress_runners(
        *runtime_scene, *grid, stress_target_count);
    ...
    register_scene_enemy_intents(
        candidate_enemy_intent, *runtime_scene,
        stress_target_count > 0 ? stress_acquisition_range(grid->topology())
                                : attacker_acquisition_range);
    CrowdSteeringPlan candidate_crowd_plan = plan_crowd_steering(*runtime_scene);
    register_scene_navigation_agents(
        candidate_navigation_agents, candidate_crowd_plan.routed_actors);
    return {
...
[[nodiscard]] NavPathResult displayed_navigation_path(
    const NavPathResult& reference,
    ...
    for (const NavAgentStateSnapshot& actor : navigation.actors) {
        if (!actor.active || actor.path_status != NavPathStatus::found ||
            actor.path.empty()) {
            continue;
        }
...
[[nodiscard]] double phase_elapsed(
    std::chrono::steady_clock::time_point& marker
) noexcept {
...
[[nodiscard]] bool health_target_alive(
    const HealthSnapshot& health,
    const EntityUuid target
) noexcept {
...
struct CombatCommandAdapter {
    ...
    [[nodiscard]] CombatCommand translate(
        ...
        if (resolved_aim) {
            ...
            if (!submitted_aim ||
                delta_x * delta_x + delta_y * delta_y > changed_epsilon_squared) {
        ...
        if (const std::optional<Vec2> movement =
                normalized_direction(world_movement_direction)) {
    ...
    [[nodiscard]] std::optional<Vec2> aim_direction() const noexcept {
        return submitted_aim;
    }
};

[[nodiscard]] std::optional<Vec2> pointer_canvas_point(
    const AimInput& aim,
    ...
    const CanvasViewport viewport = compute_canvas_viewport(
        GetScreenWidth(), GetScreenHeight(), canvas_width, canvas_height);
    if (!(viewport.scale > 0.0F) || aim.pointer_screen_x < viewport.x ||
        aim.pointer_screen_y < viewport.y ||
        aim.pointer_screen_x > viewport.x + viewport.width ||
...
void draw_pixel_crosshair(
    const Vec2& canvas_position,
    const int canvas_width,
    const int canvas_height,
    const bool firing
) {
    const int x = std::clamp(static_cast<int>(std::round(canvas_position.x)),
                             0, canvas_width - 1);
    const int y = std::clamp(static_cast<int>(std::round(canvas_position.y)),
                             0, canvas_height - 1);
...
[[nodiscard]] bool advance_fixed_step(
    ...
    Camera25DState& camera,
    const Vec2 camera_movement,
    const Vec2 player_facing_direction,
    ...
    if (crowd_steering) {
        ...
        if (goal && (!crowd_field.built() || crowd_field.goal() != *goal)) {
            ...
            static_cast<void>(
                crowd_field.rebuild(navigation_grid, *goal, {target.x, target.z}));
        }
    }
    ...
    for (const EnemyActorIntentSnapshot& actor : enemy_step.actors) {
        ...
        const auto perception = find_by_actor(
            enemy_perceptions, actor.actor, &EnemyPerception::actor);
        if (perception == enemy_perceptions.end()) {
            throw std::logic_error{
                "Enemy navigation lost its copied perception request."};
        }
    ...
    for (const EnemyActorIntentSnapshot& actor : enemy_step.actors) {
        ...
        if (!field_steered) {
            navigation = find_by_actor(
                navigation_motions, actor.actor, &NavAgentMotion::actor);
        ...
        if (actor.state == EnemyIntentState::pursuing) {
            desired = field_steered
                          ? field_direction(actor, perception->actor_position)
                          : navigation->movement_direction;
        }
        if (actor.state == EnemyIntentState::pursuing && desired.x == 0.0F &&
            desired.y == 0.0F) {
    ...
    const RuntimeSceneTickResult scene_tick =
        scene.tick({
                       .world_direction = dodge.active ? dodge.direction
                                                        : movement.world_direction,
                       .presentation_direction =
                           dodge.active
                               ? world_ground_direction_to_camera(dodge.direction, camera)
                               : player_facing_direction,
                       .speed_multiplier = dodge.active
    ...
    for (const ProjectileStateSnapshot& projectile : projectile_step.active) {
        const std::optional<RuntimeSceneSegmentHit> hit = scene.cast_segment(
            {projectile.previous_position.x, projectile.previous_position.z},
    ...
    const std::vector<ProjectileImpactEvent> projectile_impacts =
...
[[nodiscard]] bool valid(const ApplicationConfig& config) noexcept {
    ...
    const bool valid_automated_direction =
        !config.automated_movement ||
        (std::isfinite(config.automated_movement_direction.x) &&
    ...
    const bool valid_automated_aim =
        ...
        (std::isfinite(config.automated_aim_direction.x) &&
         ...
         (config.automated_aim_direction.x != 0.0F ||
          config.automated_aim_direction.y != 0.0F));
    return config.window_width > 0 && config.window_height > 0 &&
           ...
           valid_automated_direction && valid_automated_aim &&
           config.fixed_update_hz > 0.0 && config.max_frames >= 0 &&
...
[[nodiscard]] CanvasRect sprite_canvas_rect(
    const EntityBlueprint& entity,
    const Camera25DState& camera,
    const ApplicationConfig& config
) {
    ...
    for (const GroundArea& area : ground.areas) {
        if (area.kind == GroundAreaKind::elevation) {
            queue.submit_ground(project_ground_quad(
                ground_id++, area.bounds, area.elevation, {48, 103, 78, 238}, camera));
        }
    }
}

void submit_scene_sprites(
    RenderQueue2D& queue,
    ...
    for (const RenderItem2D& item : render_items) {
        ...
        const DepthSlicePlan plan = plan_depth_slices(
            item.transform.position.z, item.sprite.depth_span,
...
void submit_projectile_sprites(
    RenderQueue2D& queue,
    const ProjectileSimulationSnapshot& snapshot,
    const Camera25DState& camera,
    ...
    for (const ProjectileStateSnapshot& projectile : snapshot.active) {
        ...
        const float rotation = std::atan2(
                                   projected.position.y - projected_previous.position.y,
        ...
        const std::uint64_t stable_id =
            projectile_stable_id_base + projectile.projectile_id * 4U;
        queue.submit(SpriteSubmission2D{
            .stable_id = stable_id,
        ...
        if (projectile.weapon == WeaponKind::needle_pistol &&
            projectile.lifetime_ticks_remaining + 1U ==
                needle_pistol.projectile_lifetime_ticks) {
            queue.submit(SpriteSubmission2D{
...
void submit_navigation_grid(
    RenderQueue2D& queue,
    const NavGridSnapshot& navigation,
    const Camera25DState& camera,
    ...
    for (std::size_t index = 0; index < navigation.cells.size(); ++index) {
        ...
        const float left = std::max(
            navigation.bounds.x, cell.center.x - navigation.cell_size * 0.5F) + inset;
        const float near_z = std::max(
            navigation.bounds.z, cell.center.y - navigation.cell_size * 0.5F) + inset;
        const float right = std::min(
            maximum_x, cell.center.x + navigation.cell_size * 0.5F) - inset;
        const float far_z = std::min(
            maximum_z, cell.center.y + navigation.cell_size * 0.5F) - inset;
        if (!(right > left) || !(far_z > near_z)) {
            continue;
        }
        const ColorRgba8 tint = cell.walkable
                                    ? ColorRgba8{74, 222, 190, 65}
                                    : ColorRgba8{224, 74, 86, 150};
        queue.submit_ground(project_ground_quad(
            navigation_id_base + index,
            {left, near_z, right - left, far_z - near_z},
            cell.elevation + 0.20F, tint, camera));
    }
}

void submit_navigation_path(
    RenderQueue2D& queue,
    const NavGridSnapshot& navigation,
    ...
    for (std::size_t index = 0; index < path.cells.size(); ++index) {
        ...
        if (path_cell.column < 0 || path_cell.row < 0 ||
            path_cell.column >= navigation.columns || path_cell.row >= navigation.rows) {
            continue;
        }
        const std::size_t cell_index = static_cast<std::size_t>(path_cell.row) *
                                           static_cast<std::size_t>(navigation.columns) +
        ...
        const ColorRgba8 tint = start
                                    ? ColorRgba8{74, 222, 190, 235}
...
int run_application(const ApplicationConfig& requested_config) {
    ...
    try {
        ...
        if (config.initial_editor_enemy_stress_count > 0) {
            ...
            log(LogLevel::info,
                "Enemy stress scene initialized with " +
                    std::to_string(stress.total()) + " total Runner(s), including " +
                    std::to_string(stress.spawned) +
    ...
    debug_visuals.set_channel_selected(
        DebugChannel::navigation_grid, config.start_with_navigation_grid_debug);
    debug_visuals.set_channel_selected(
        DebugChannel::navigation_path, config.start_with_navigation_path_debug);
    ...
    if (renderdoc_capture.available()) {
        log(LogLevel::info,
            "RenderDoc detected: automatic captures every "
                + std::to_string(config.renderdoc_capture_interval_seconds) + " seconds.");
    }
#endif
    ...
    while (!WindowShouldClose() && !close_requested &&
        ...
        const Vec2 camera_movement{
            ...
            config.automated_movement ? config.automated_movement_direction.y
                                      : input.move_depth,
        };
        const GroundMovement25D frame_movement =
            camera_ground_movement(camera_movement, world_camera);
        const GroundMovement25D idle_facing =
            camera_ground_movement({0.0F, 1.0F}, world_camera);
        ...
        for (std::uint32_t step = 0;
            ...
            const bool interact_spent =
                advance_fixed_step(*scene, *navigation_grid, navigation_agents,
                               ...
                               enemy_observations,
                               projectile_observations, health_observations,
...
#endif
        if (refresh_digest && simulated_ticks != ticks_before_frame &&
            !gameplay_digest_failed) {
            try {
                ...
                log(LogLevel::error,
                    std::string{"Gameplay state digest failed: "} + error.what());
            }
        }
#if IC2DE_ENABLE_DEVELOPMENT_TOOLS
        ...
        phase_totals.peak_visible_sprites = std::max(
        ...
        if (editor_actions.apply_document_to_running_scene && scene_editor) {
            try {
                ...
                EditorRuntimeSceneCandidate candidate =
                    make_editor_runtime_scene_candidate(
                        scene_editor->runtime_copy(), textures, 0);
        ...
        if (editor_actions.enemy_stress_target_count && scene_editor) {
            try {
                const std::size_t requested =
                    *editor_actions.enemy_stress_target_count;
                EditorRuntimeSceneCandidate candidate =
                    make_editor_runtime_scene_candidate(
                ...
                commit_editor_runtime(std::move(candidate));
                log(LogLevel::info,
                    "Enemy stress scene rebuilt with " +
                        std::to_string(stress.total()) + " total Runner(s), including " +
                        std::to_string(stress.spawned) + " runtime copy/copies; requested " +
                        std::to_string(requested) +
                        (enemy_attack_damage_enabled
                             ? "; normal player damage restored."
    ...
    // one or two sectors is being funnelled, and the counts say by how much.
    {
        ...
        if (crowd.size() >= 64) {
            ...
            for (std::size_t index = 0; index < sectors.size(); ++index) {
                report += std::string{names[index]} + " " +
                          std::to_string(sectors[index]) + "  ";
            }
                    ...
                    for (std::int32_t dc = -1; dc <= 1; ++dc) {
                        const std::uint64_t key =
                            (static_cast<std::uint64_t>(
                                 static_cast<std::uint32_t>(column + dc)) << 32U) |
    ...
    if (fixed_step_phases.ticks > 0) {
        ...
        log(LogLevel::info,
            "Fixed step averages over " + std::to_string(fixed_step_phases.ticks) +
                ...
                " ms, scene tick " + std::to_string(tick_ms(fixed_step_phases.scene_tick)) + " ms.");
    }
    if (phase_totals.frames > 0) {
        ...
        log(LogLevel::info,
            "Frame phase averages over " + std::to_string(phase_totals.frames) +
                ...
                " ms, frame snapshots " +
                std::to_string(average_ms(phase_totals.frame_snapshots)) + " ms.");
        log(LogLevel::info,
            "Visible sprites: mean " +
    ...
    if (projectile_spawn_count < config.minimum_automated_projectile_spawns) {
        log(LogLevel::error,
            "Automated held-fire validation spawned only " +
                std::to_string(projectile_spawn_count) + " projectile(s); expected at least " +
                std::to_string(config.minimum_automated_projectile_spawns) + ".");
        return 12;
    }
    if (config.minimum_automated_projectile_spawns > 0) {
        log(LogLevel::info,
            "Automated held-fire validation passed with " +
    ...
    if (projectile_impact_count < config.minimum_automated_projectile_impacts) {
        log(LogLevel::error,
            "Automated projectile-impact validation resolved only " +
                std::to_string(projectile_impact_count) + " impact(s); expected at least " +
                std::to_string(config.minimum_automated_projectile_impacts) + ".");
        return 13;
    }
    if (config.minimum_automated_projectile_impacts > 0) {
        log(LogLevel::info,
            "Automated projectile-impact validation passed with " +
    ...
    if (target_death_count < config.minimum_automated_target_deaths) {
        log(LogLevel::error,
            "Automated target-health validation observed only " +
                std::to_string(target_death_count) + " death(s); expected at least " +
    ...
    if (health_observations.retired_crowd_actor_count <
        ...
        log(LogLevel::error,
            "Automated crowd-kill validation retired only " +
    ...
    if (config.minimum_automated_crowd_actor_retirements > 0) {
        log(LogLevel::info,
            "Automated crowd-kill validation passed with " +
                std::to_string(health_observations.retired_crowd_actor_count) +
    ...
    if (dodge.started_count < config.minimum_automated_dodge_starts) {
        log(LogLevel::error,
            "Automated dodge validation observed only " +
                std::to_string(dodge.started_count) + " start(s); expected at least " +
    ...
    if (config.expected_automated_dodge_distance > 0.0F) {
        ...
        const float distance_error = std::abs(
            combat_observations.dodge_distance_travelled -
            config.expected_automated_dodge_distance);
        if (distance_error > distance_tolerance) {
            log(LogLevel::error,
                "Automated dodge movement travelled " +
    ...
    if (config.minimum_automated_dodge_starts > 0) {
        log(LogLevel::info,
            "Automated dodge validation passed with " +
                std::to_string(dodge.started_count) +
                " fixed-tick start(s), an observed invulnerability window, completed duration, and " +
                std::to_string(combat_observations.dodge_distance_travelled) +
    ...
    if (enemy_result.acquisition_count < config.minimum_automated_enemy_acquisitions) {
        log(LogLevel::error,
            "Automated enemy-intent validation observed only " +
    ...
    if (enemy_observations.distance_travelled < config.minimum_automated_enemy_distance) {
        log(LogLevel::error,
            "Automated moving-attacker validation travelled only " +
    ...
    if (enemy_observations.damage_applied_to_player < config.minimum_automated_player_damage) {
        log(LogLevel::error,
            "Automated moving-attacker validation applied only " +
    ...
    if (config.require_automated_zero_player_damage &&
        ...
        log(LogLevel::error,
            "Automated harmless-enemy validation applied " +
                std::to_string(enemy_observations.damage_applied_to_player) +
    ...
    if (config.require_automated_zero_player_damage) {
        log(LogLevel::info,
            "Automated harmless-enemy validation passed with zero player damage.");
    }
    ...
    if (navigation_result.actors.size() < config.minimum_automated_navigation_agents) {
        log(LogLevel::error,
            "Automated navigation validation retained only " +
    ...
    if (navigation_result.total_search_count <
        config.minimum_automated_navigation_searches) {
        log(LogLevel::error,
            "Automated navigation validation performed only " +
    ...
    if (waypoint_advances < config.minimum_automated_navigation_waypoint_advances) {
        log(LogLevel::error,
            "Automated navigation validation advanced only " +
                std::to_string(waypoint_advances) +
