...
int camera_failures = 0;

void camera_expect(const bool condition, const std::string_view message) {
    if (!condition) {
...
int run_editor_camera_tests() {
    ...
    camera_expect(!camera.detached(), "A new editor camera must follow the gameplay camera.");
    const ic2d::Camera25DState attached = camera.resolve(gameplay);
    camera_expect(near(attached.focus.x, gameplay.focus.x) &&
                      near(attached.focus.z, gameplay.focus.z) &&
                      near(attached.zoom, gameplay.zoom),
    ...
    camera_expect(!camera.detached(), "An empty drag must not detach the view.");
    ...
    camera_expect(camera.detached(), "Panning must detach the view from the gameplay camera.");
    ic2d::Camera25DState view = camera.resolve(gameplay);
    camera_expect(near(view.focus.x, 100.0F - 10.0F),
                  "A horizontal drag must move the focus against the pointer at scale.");
    camera_expect(near(view.focus.z, 40.0F),
    ...
    camera_expect(near(view.focus.x, 90.0F) && near(view.focus.z, 40.0F),
                  "A detached view must ignore later gameplay camera movement.");
    camera_expect(near(view.pitch_degrees, moved.pitch_degrees) &&
    ...
    camera.attach();
    view = camera.resolve(moved);
    camera_expect(!camera.detached() && near(view.focus.x, 900.0F) && near(view.focus.z, 900.0F),
    ...
    camera_expect(near(zoom_camera.resolve(gameplay).zoom, 1.15F),
                  "One wheel notch must scale zoom by a fixed proportion.");
    zoom_camera.zoom(-1.0F, gameplay);
    camera_expect(near(zoom_camera.resolve(gameplay).zoom, 1.0F),
                  "Zooming out by the same notch must return to the original scale.");
    zoom_camera.zoom(1000.0F, gameplay);
    camera_expect(near(zoom_camera.resolve(gameplay).zoom, ic2d::EditorCamera::maximum_zoom),
                  "Zoom must saturate at the maximum rather than run away.");
    zoom_camera.zoom(-10000.0F, gameplay);
    camera_expect(near(zoom_camera.resolve(gameplay).zoom, ic2d::EditorCamera::minimum_zoom),
                  "Zoom must saturate at the minimum rather than invert.");

    // Framing centres the ground position without adopting its elevation.
    ic2d::EditorCamera frame_camera;
    frame_camera.frame({-250.0F, 68.0F, 310.0F}, gameplay);
    view = frame_camera.resolve(gameplay);
    camera_expect(near(view.focus.x, -250.0F) && near(view.focus.z, 310.0F),
                  "Framing must centre the view on the requested ground position.");
    camera_expect(near(view.focus.y, gameplay.focus.y),
                  "Framing must not tilt the view by adopting entity elevation.");
    camera_expect(near(view.zoom, gameplay.zoom), "Framing must not change zoom.");
    ...
    camera_expect(!guarded.detached(),
    ...
    // along world X and Z.
    ic2d::Camera25DState yawed = gameplay;
    yawed.yaw_degrees = 90.0F;
    ic2d::EditorCamera yawed_camera;
    yawed_camera.pan({20.0F, 0.0F}, yawed);
    view = yawed_camera.resolve(yawed);
    camera_expect(near(view.focus.x, yawed.focus.x) && near(view.focus.z, yawed.focus.z + 10.0F),
                  "Panning must follow the camera's screen axes through yaw.");

    return camera_failures;
}
