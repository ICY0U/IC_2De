...
void test_acquires_pursues_attacks_and_respects_cooldown() {
    ...
    expect(snapshot.tick == 1 && snapshot.acquisition_count == 1 && snapshot.actors.size() == 1 &&
               snapshot.actors.front().acquired &&
...
void test_order_is_canonical_and_target_death_stops_intent() {
    ...
    expect(first.register_actor(definition(300, 100)) && first.register_actor(definition(200, 100)),
    ...
    expect(std::ranges::all_of(inactive.actors,
                               [](const auto& actor) {
...
void test_rejects_invalid_perception_and_reset_restores_definitions() {
    ...
    expect(intent.register_actor(definition(200, 100)), "The valid reset fixture must register.");
    ...
    expect(reset.tick == 0 && reset.next_event_sequence == 1 && reset.acquisition_count == 0 &&
               reset.attack_count == 0 && reset.actors.size() == 1 &&
               !reset.actors.front().acquired &&
