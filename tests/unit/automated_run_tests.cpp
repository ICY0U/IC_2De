...
int automated_run_failures = 0;

void automated_run_expect(const bool condition, const std::string_view message) {
    if (!condition) {
        std::cerr << "FAIL: " << message << '\n';
        ++automated_run_failures;
    }
}
...
void test_interactive_run_ignores_gameplay_checks() {
    ...
    automated_run_expect(
        ic2d::evaluate_automated_run(observations) == ic2d::AutomatedRunExit::passed,
...
void test_exit_codes_are_stable() {
    automated_run_expect(ic2d::exit_code(ic2d::AutomatedRunExit::passed) == 0,
                         "A passing run must exit zero.");
    automated_run_expect(ic2d::exit_code(ic2d::AutomatedRunExit::texture_lifetime_failed) == 6,
                         "Texture lifetime failure must stay exit code 6.");
    automated_run_expect(ic2d::exit_code(ic2d::AutomatedRunExit::ground_map_incomplete) == 7,
                         "Ground-map failure must stay exit code 7.");
    automated_run_expect(ic2d::exit_code(ic2d::AutomatedRunExit::smoke_capture_failed) == 8,
                         "Smoke capture failure must stay exit code 8.");
    automated_run_expect(ic2d::exit_code(ic2d::AutomatedRunExit::physics_incomplete) == 9,
                         "Physics failure must stay exit code 9.");
    automated_run_expect(ic2d::exit_code(ic2d::AutomatedRunExit::animation_incomplete) == 10,
                         "Animation failure must stay exit code 10.");
    automated_run_expect(ic2d::exit_code(ic2d::AutomatedRunExit::locomotion_incomplete) == 11,
...
int run_automated_run_tests() {
    test_complete_run_passes();
    test_interactive_run_ignores_gameplay_checks();
    test_ground_map_gaps_are_reported();
    test_physics_gaps_are_reported();
    test_animation_gaps_are_reported();
    test_resource_and_capture_failures();
    test_gameplay_gaps_outrank_resource_gaps();
    test_exit_codes_are_stable();

    if (automated_run_failures == 0) {
        std::cout << "Automated run verdict tests passed.\n";
    }
    return automated_run_failures;
}
