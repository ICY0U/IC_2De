...
// Shared by the cases that assert about aiming with nothing to assist toward.
const std::span<const ic2d::AimTarget> no_targets{};

} // namespace

TEST_CASE("a pointer is absolute") {
    ...
    CHECK_MESSAGE((snapshot.aiming && snapshot.pointer_source),
                  "A pointer away from the actor must count as aiming.");
    CHECK_MESSAGE((near(snapshot.direction.x, 1.0F) && near(snapshot.direction.y, 0.0F)),
                  "Pointer aim must resolve exactly toward the pointer.");
    CHECK_MESSAGE((!snapshot.turning), "Pointer aim must never be rate limited.");
    CHECK_MESSAGE((near(snapshot.distance, 100.0F)),
                  "Pointer aim distance must be the ground distance to the pointer.");
    CHECK_MESSAGE((near(snapshot.aim_point.x, 100.0F) && near(snapshot.aim_point.z, 0.0F)),
    ...
    CHECK_MESSAGE((near(snapshot.origin.x, aiming.config().muzzle.forward) &&
    ...
    CHECK_MESSAGE((near(spawn.x, snapshot.origin.x) && near(spawn.y, snapshot.origin.y) &&
    ...
    CHECK_MESSAGE((near(degenerate.x, 5.0F) && near(degenerate.z, 5.0F) &&
                   near(degenerate.y, aiming.config().muzzle.height)),
                  "A zero aim direction must not divide by zero.");
}

TEST_CASE("a pointer resting on the actor has no direction to give") {
    ...
    CHECK_MESSAGE((!held.aiming), "A pointer on the actor must not count as aiming.");
    CHECK_MESSAGE((near(held.direction.x, 1.0F) && near(held.direction.y, 0.0F)),
                  "A pointer on the actor must hold the previous direction.");
}

TEST_CASE("a stick at rest must not drift the aim") {
    // A stick at rest must not drift the aim.
    ic2d::Aiming aiming;
    ic2d::AimingInputs inputs = at_origin();
    inputs.stick_world = {0.10F, 0.05F};
    const ic2d::AimingSnapshot& snapshot = aiming.resolve(inputs, no_targets);
    CHECK_MESSAGE((!snapshot.aiming), "A stick inside the dead zone must not aim.");
    CHECK_MESSAGE((near(snapshot.direction.x, 0.0F) && near(snapshot.direction.y, 1.0F)),
                  "A resting stick must hold the default facing.");
}

TEST_CASE("a stick is a rate control") {
    ...
    CHECK_MESSAGE((first.aiming && first.turning),
    ...
    const float allowed = aiming.config().stick_turn_degrees_per_second * (1.0F / 60.0F) + 0.01F;
    CHECK_MESSAGE((turned > 0.0F && turned <= allowed),
    ...
    CHECK_MESSAGE((frames < 240), "A held stick must reach its requested direction.");
    CHECK_MESSAGE((near(aiming.snapshot().direction.x, 1.0F, 0.01F) &&
                   near(aiming.snapshot().direction.y, 0.0F, 0.01F)),
                  "Turning must settle exactly on the requested direction.");
}

TEST_CASE("partial deflection turns proportionally slower") {
    ...
    const float slow = degrees_between({0.0F, 1.0F}, partial.resolve(light, no_targets).direction);
    CHECK_MESSAGE((slow > 0.0F && slow < fast),
                  "A light stick push must turn slower than a full one.");
}

TEST_CASE("assist is a nudge with a bound, not a lock") {
    ...
    CHECK_MESSAGE((snapshot.assisted_target.has_value() && snapshot.assisted_target->value == 77),
    ...
    CHECK_MESSAGE((to_raw > 0.0F), "Assist must actually move the aim.");
    CHECK_MESSAGE((to_exact > 0.0F),
                  "Assist must not snap the aim onto the target: the player keeps authority.");
    CHECK_MESSAGE((near(snapshot.distance, std::sqrt(100.0F * 100.0F + 12.0F * 12.0F), 0.01F)),
                  "An assisted aim point must sit at the target's range.");
}

TEST_CASE("a target outside the cone or beyond range must be ignored entirely") {
...
TEST_CASE("automation states an exact direction") {
    ...
    CHECK_MESSAGE((!snapshot.turning && !snapshot.assisted_target.has_value()),
    ...
    CHECK_MESSAGE((near(paced.direction.x, snapshot.direction.x) &&
                   near(paced.direction.y, snapshot.direction.y)),
                  "A direct frame must not depend on the frame duration.");
}

TEST_CASE("a bad sample must hold the last good aim rather than snapping") {
    ...
    CHECK_MESSAGE((near(held.direction.x, 0.0F) && near(held.direction.y, -1.0F)),
                  "An invalid frame must hold the previous direction.");
    CHECK_MESSAGE((!held.aiming), "An invalid frame must not report an aim.");
}
