...
TEST_CASE("file texture reload preserves handle and last good content") {
    ...
    CHECK_MESSAGE((IsWindowReady()), "The texture reload test requires a hidden graphics context.");
    ...
    if (IsWindowReady()) {
        ...
        CHECK_MESSAGE((initial && initial->width == 1122 && initial->height == 1402 &&
                       initial->revision == 1),
        ...
        CHECK_MESSAGE((observing.reloaded == 0),
                      "A new file stamp must remain stable for one poll before upload.");
        CHECK_MESSAGE((reloaded.watched == 1 && reloaded.changed == 1 && reloaded.reloaded == 1 &&
                       reloaded.failed == 0),
                      "A stable file change must reload exactly one watched texture.");
        CHECK_MESSAGE((textures.alive(handle) && replacement && replacement->width == 1536 &&
        ...
        CHECK_MESSAGE((rejected.changed == 1 && rejected.reloaded == 0 && rejected.failed == 1),
                      "An invalid stable replacement must report one failed reload.");
        CHECK_MESSAGE((textures.alive(handle) && retained && retained->width == 1536 &&
        ...
        CHECK_MESSAGE((textures.loaded_texture_count() == 0),
